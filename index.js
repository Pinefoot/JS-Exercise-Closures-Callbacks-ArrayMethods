...
function processSum(numberList, callback) {
  let sumSum = numberList.reduce( function(a, b){
    return a + b;
  }, 0);
  return callback(sumSum);
...
function processContains(item, list, callback) {
  const isThisTrue = list.includes(item);
return callback(isThisTrue);
...
function getFullNames(runners) {
  const fullName = [];
    runners.forEach(function(names){
      fullName.push(`${names.last_name}, ${names.first_name}`);
    });
    return fullName;
}
